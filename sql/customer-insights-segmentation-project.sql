...
SELECT Segment, (`Customer Name`), ROUND(SUM(Sales), 2) AS `Total Revenue` -- Select the customer name and calculate their total revenue using the sum of sales.
...
LIMIT 10; -- Limit the results to the top 10 highest spending customers.
...
SELECT Segment, DATE_FORMAT(STR_TO_DATE(`Order Date`, '%m/%d/%Y'), '%Y-%m') AS `Month`, -- Format the Date because it was formatted as STR.
...
SELECT Segment, COUNT(`Order ID`) AS `Amount of Orders` -- Select the amount of orders per Region and Segment
FROM projects.superstore
GROUP BY Segment
